...
## The One-Sentence Problem {#one-sentence-problem}
...
## Real-Life Analogy: Picking a Lunch Spot {#real-life-analogy}
...
## The Five Strategies (all in `compare_exploration.py`) {#five-strategies}

### 1. ε-greedy — the default, and it's *dithering*, not exploring {#epsilon-greedy}
...
### 2. Optimistic initialisation — "innocent until proven boring" {#optimistic-initialization}
...
### 3. UCB-style action selection — bonus in the *choice*, not the *reward* {#ucb-selection}
...
### 4. Count-based **reward** bonus — curiosity, the classic version {#count-based-reward}
...
### 5. Prediction-error **reward** bonus — curiosity, the ICM/RND version {#prediction-error-reward}
...
## The Two Test Tasks {#two-test-tasks}
...
## What Actually Happens (run it and see) {#what-happens}
