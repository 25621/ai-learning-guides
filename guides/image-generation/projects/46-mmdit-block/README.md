...
- **The Big Idea:** In standard diffusion models, text conditioning is one-way: the image looks at the text, but the text encoder is frozen and doesn't look at the image. MMDiT (Multi-Modal Diffusion Transformer) lets text and image tokens talk to each other in both directions within the same attention blocks, creating a joint understanding.
- **Analogy:** Imagine a writer and an illustrator working on a comic book. In cross-attention, the illustrator reads the writer's script but can't talk to them. In MMDiT, they sit at the same table and discuss the project together, resulting in a much better match between the words and the drawing.
- **Example:** If you prompt "a red book on a blue table," standard cross-attention might get confused and draw a blue book on a red table. MMDiT's joint attention keeps the colors tied to the correct objects, improving prompt adherence.
