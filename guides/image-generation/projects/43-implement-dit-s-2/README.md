...
| `train_dit.py` | Class-conditional training — project 24's diffusion math, unchanged |
...
class-conditional CIFAR target from the guide is this plus project 25's
loader and project 39's VAE.
...
plus a learned position vector per token (project 44 replaces exactly this
...
(project 24), applied at every block. This is the entire conditioning
...
deliberate, because project 44 needs to rotate `q` and `k` before the dot
product (RoPE) and project 46 needs to concatenate two token streams, and
...
sampler, EMA — the training script imports them from project 24 untouched.
...
60k images that purchase is still in progress. Project 48 turns this
