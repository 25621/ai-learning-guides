"""Shared matplotlib styling for the RL Phase-1 figures.
...
Imported by projects 02, 04, and 05 via sys.path.
"""
...
def style_axes(ax):
    """Apply the shared chrome to an externally created axes."""
    ax.set_facecolor(SURFACE)
    for side in ("top", "right"):
        ax.spines[side].set_visible(False)
    for side in ("left", "bottom"):
