...
### Projector {#projector}
The small network — often a single linear layer or a two-layer [MLP](/shared/glossary/#mlp) — that maps one [modality](/shared/glossary/#modality)'s feature vectors into the space another model expects, so a frozen image encoder's output can be fed straight into a language model as if it were word [embeddings](/shared/glossary/#embedding). It does no heavy reasoning; it just *translates* coordinates, like an adapter cable that reshapes one plug into another without changing the signal. This is the entire fusion mechanism in [LLaVA](/shared/glossary/#vlm): freeze the vision encoder, freeze the LLM, and train only this projector to align the two spaces — a striking demonstration that a trivial connector plus good data can rival far fancier modules like the [Q-Former](/shared/glossary/#q-former). The catch is that all the image information must squeeze through this one thin bridge, so it can become a bottleneck on detail-heavy tasks.
