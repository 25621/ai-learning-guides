...
**What it does (the effect).** Where the [dot product](/shared/glossary/#dot-product) measures how *aligned* two vectors are, the cross product hands you the axis they are *not* aligned along — the direction sticking out of the flat sheet that the two of them span. Its length carries a meaning too: it equals the area of the parallelogram the two vectors frame, so it is largest when they sit at a clean right angle and shrinks to zero when they point the same way (parallel arrows frame no area, so their cross product is the all-zeros vector). Analogy: point the fingers of your right hand along the first vector and curl them toward the second — your thumb now points along the cross product (the "right-hand rule"). It is the same motion as a screwdriver: twist the handle from one direction toward another and the screw drives in or out along the perpendicular axis.

**Why [Plücker coordinates](/shared/glossary/#plücker-coordinates) use it.** A line floating in 3D needs more than a direction to be pinned down — two parallel rails point the same way yet sit in different places. Taking the cross product of a point on the line with the line's direction produces the line's *moment*, a perpendicular vector that comes out the same for every point on that line but different for a parallel neighbour — exactly the extra fingerprint that tells the two rails apart.
...
### UCF-101 {#ucf-101}
A widely used action-recognition video dataset of about 13,000 short YouTube clips spanning 101 human-action categories (playing guitar, applying makeup, bench-pressing, and so on); the "101" is simply the number of action classes. It is small and low-resolution by modern standards, which is exactly why it became a common testbed for early video generation — you can train on it without a data-center. It shows up constantly in pre-diffusion video-GAN papers as the dataset everyone reported numbers on.
