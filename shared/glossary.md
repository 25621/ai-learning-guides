...
### Cross-Entropy Method Model Predictive Control (CEM-MPC) {#cross-entropy-method-model-predictive-control-cem-mpc}
A [planning](/shared/glossary/#planning) algorithm that combines [model predictive control](/shared/glossary/#mpc) with the [Cross-Entropy Method](/shared/glossary/#cem). At each step, it uses a [dynamics model](/shared/glossary/#dynamics-model) to simulate into the future, but instead of trying actions entirely at random, it uses CEM to iteratively search for the most promising action sequences. It executes the first action of the best sequence, then replans at the next step. Analogy: a chess player who looks ahead a few moves, initially considering a wide variety of plays, but quickly focuses their mental simulation only on the variations that seem the most promising, makes one move, and then repeats the process.
...
### Dynamics head {#dynamics-head}
In architectures like [MuZero](/shared/glossary/#muzero), a neural network branch (or "head") that learns to step a [latent](/shared/glossary/#latent-space) state forward in time. Given the current hidden state and a chosen action, it predicts what the next hidden state and immediate [reward](/shared/glossary/#reward-function) will be, acting as the system's [latent dynamics](/shared/glossary/#latent-dynamics) model. Analogy: the part of your brain that imagines how a pile of blocks will shift if you pull one out, without needing to physically touch them.

### Dynamics model {#dynamics-model}
A learned stand-in for the environment's rules: given the current state and an action, it predicts the next state (and usually the [reward](/shared/glossary/#reward-function)) — written `f(s, a) → s′, r`. Once you have it, you can simulate experience cheaply instead of paying for real environment steps, which is the whole basis of [model-based RL](/shared/glossary/#model-based-rl). It is called a "dynamics model" because "dynamics" refers to how things change over time — the rules of motion or progression. Unlike a generic "model" (which might just statically classify an image or translate a sentence), a dynamics model specifically predicts the *future state* of an environment as it evolves. The catch is *compounding error*: each predicted state feeds the next prediction, so small mistakes snowball over a long [rollout](/shared/glossary/#rollout) — which is why model-based methods keep imagined rollouts short or train an [ensemble](/shared/glossary/#ensemble) to sense when the model is unsure. Analogy: a weather model that is accurate one hour out but drifts badly a week out. It can be a plain forward network, an ensemble, or a [latent dynamics](/shared/glossary/#latent-dynamics) model that predicts in a compressed feature space rather than over raw pixels.
...
Predicting how an environment evolves in a compressed *latent* code rather than in raw observations: an encoder squeezes each high-dimensional observation (say an image) into a small [latent](/shared/glossary/#latent-space) vector, and the model learns to step *that* code forward given an action — `z, a → z′`. It is called "latent dynamics" rather than just a "latent model" or "latent space" to emphasize that it models how things *change* over time ("dynamics"). A latent space is just a static compressed map, but latent dynamics is the engine that moves you through that map step by step. This is far cheaper than predicting every pixel and lets the model ignore visual detail that does not affect decisions, spending its capacity on what matters for control. Analogy: planning a road trip on a subway-style map of dots and lines instead of a full satellite photo — the stripped-down map keeps exactly the structure you need to navigate. It is the engine inside [DreamerV3](/shared/glossary/#dreamerv3) and [TD-MPC2](/shared/glossary/#td-mpc2), letting them "imagine" long [rollouts](/shared/glossary/#rollout) quickly.
...
### MPC {#mpc}
Model Predictive Control — a control strategy that, at every step, looks a short distance into the future, picks the best plan over that window, executes only the *first* action, then throws the rest away and re-plans from scratch next step with updated information. The "predictive" part uses a [dynamics model](/shared/glossary/#dynamics-model) to simulate where candidate action sequences would lead; "finite-horizon" means it plans only a few steps ahead, not to the end of the task; and re-planning every step (the "receding horizon") keeps it robust to model error and surprises. Common ways to search for the best action sequence are [random shooting](/shared/glossary/#random-shooting), the [Cross-Entropy Method](/shared/glossary/#cem), and [MPPI](/shared/glossary/#mppi). Analogy: driving in fog — you plan only as far as your headlights reach, commit to the next moment, then re-plan as more road appears.
...
**Analogy:** Imagine trying to keep a car in the center of its lane. The **Proportional** part is how much you turn the steering wheel when you drift off-center (drift right, turn left). The **Integral** part kicks in if there is a crosswind pushing you to one side, and you realize you've been drifting for a while, so you turn the wheel slightly more to compensate. The **Derivative** part is how you straighten the wheel as you return to the center to avoid oversteering and weaving back and forth.
**Example:** Cruise control in a car uses a PID controller to maintain a constant speed. When going up a hill, the speed drops (error), so the **Proportional** term presses the gas. As the speed stays too low for a while, the **Integral** term presses the gas even harder. As the car nears the target speed, the **Derivative** term eases off the gas so the car doesn't overshoot the speed limit.
...
In architectures like [MuZero](/shared/glossary/#muzero), a neural network branch that looks at a [latent](/shared/glossary/#latent-space) state and predicts the [value](/shared/glossary/#value-function) (who is winning) and the [policy](/shared/glossary/#policy) (what moves are good to play next). It acts as the evaluator for states generated by the [dynamics head](/shared/glossary/#dynamics-head). Analogy: the instinct of a seasoned game player who can glance at a board position and immediately sense both how strong their position is and which few moves are worth considering.
...
In architectures like [MuZero](/shared/glossary/#muzero) (where it is often called the representation function or head), the neural network branch that takes the raw environment observation (like pixels or a board state) and encodes it into a compressed, abstract [latent](/shared/glossary/#latent-space) state. This abstract state throws away irrelevant details and keeps only what is necessary for [planning](/shared/glossary/#planning). Analogy: glancing at a messy desk and remembering only where your keys are, stripping away the visual noise to focus on what matters.
