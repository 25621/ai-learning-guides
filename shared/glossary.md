...
### SAC {#sac}
**Soft Actor-Critic (SAC)** is an [off-policy](/shared/glossary/#off-policy) [actor-critic](/shared/glossary/#actor-critic) reinforcement learning algorithm designed for environments with continuous action spaces (like controlling the joint angles of a robotic arm). Its distinguishing feature is **entropy maximization**: instead of only maximizing the expected sum of rewards, the agent is also rewarded for acting as randomly as possible (maximizing its [entropy regularization](/shared/glossary/#entropy-regularization)). This prevents the policy from prematurely converging to a single sub-optimal strategy and encourages active exploration.
